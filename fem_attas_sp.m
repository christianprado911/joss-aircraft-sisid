...
g = @g_attas_sp;
K = @K_attas_sp;
mdlpred = @(theta) lin_euler_pred(x0, t, u, z, fgen, ggen, K, theta);

R0 = diag(var(z(t<5, :), 0, 1));
theta0 = [-1; 0; 0; 0; -1; 0; z_pre(1); u_pre(1); 0; 0; 0; 0; 0];

%% Chama o otimizador
oemopt = oemoptions();
oemopt.R = R0;
oemopt.deriv = 'fwd-diff';
fun = @(theta) oem_obj(theta, z, mdlpred, oemopt);
